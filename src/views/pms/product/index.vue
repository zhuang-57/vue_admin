      ...
      <div style="margin-top:15px">
        <el-form :inline="true" :model="listQuery" size="small" label-width="140px">
          ...
          <el-form-item label="商品分类：">
            <!--el-cascader是级联选择器 clearable表示是否支持清空选项，默认值为false -->
...
export default {
  ...
  methods: {
    ...
    handleShowSkuEditDialog(index,row){//处理显示sku
    ...
    handleSizeChange(val) {//控制尺寸变化
      this.listQuery.pageNum = 1;
      this.listQuery.pageSize = val;
      this.getList();
    },
    handleCurrentChange(val) {//处理当前更改
      this.listQuery.pageNum = val;
      this.getList();
    },
    handleSelectionChange(val) {//处理选择更改
      this.multipleSelection = val;
    },
    handlePublishStatusChange(index, row) {//处理上架状态更改
      let ids = [];
      ids.push(row.id);
      this.updatePublishStatus(row.publishStatus, ids);
    },
    handleNewStatusChange(index, row) {//处理上新状态更改
        let ids = [];
        ids.push(row.id);
        this.updateNewStatus(row.newStatus, ids);
    },
    handleRecommendStatusChange(index, row) {//处理推荐状态更改
      let ids = [];
      ids.push(row.id);
      this.updateRecommendStatus(row.recommandStatus, ids);
    },
    handleResetSearch() {//搜索商品重置
      this.selectProductCateValue = [];
      this.listQuery = Object.assign({}, defaultListQuery);
    },
    handleDelete(index, row){//控制删除
    ...
    handleUpdateProduct(index,row){//处理编辑产品
      this.$router.push({path:'/pms/updateProduct',query:{id:row.id}});
    },
    handleShowProduct(index,row){//控制查看产品
      console.log("handleShowProduct",row);
    },
    handleShowVerifyDetail(index,row){//控制显示验证详细信息
      console.log("handleShowVerifyDetail",row);
    },
    handleShowLog(index,row){//控制显示日志
      console.log("handleShowLog",row);
    },
    updatePublishStatus(publishStatus, ids) {//更新发布状态
